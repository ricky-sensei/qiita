...
print(gojita)  //{'かめはめ波': True, 'ファイナルフラッシュ': True, 'サイヤ人': True}
```
※この教材ではメモリ消費の多さをアピールする手段として、わざとdict型を使っています。なので、メモリ消費の観点では、dictは激重なので、ビット演算どうこうっていう話じゃないやないかい!っていうツッコミはなしの方向でお願いします!
※あくまで情報学の教材なので、pythonになれない人でもわかる構文にしています。

このコード、変数宣言だけで11行躯体使ってる。それだけ多くのメモリを使っているということ。
これをビット演算を使って表すと、こんな感じになる
...
ツッコミどころとしては
```
短くなったのはわかるけど、こんなコード読めるかい!
```
って感じだろうか。
そのとおりで、プログラミング言語によってある程度対策はできるけど、ビット演算を使うとコードの読みやすさや理解のしやすさが低下することもある。
昔はコンピュータの性能も今ほど良くはなかったから、ゲーム等の重い処理をするためにビット演算を使わないと、コンピュータの動作に影響する時代も会ったけど、最近はそこまで気にしなくていいほどコンピュータの性能も上がってきた。
でも、バリバリの3Dのゲーム等で、少しでも処理を軽くする手段としてビット演算が使われたりもするし、何よりセンター試験に出るので、やっぱり覚えておこう笑
